      ...
      if (characters && characters.length > 0) {
        ...
        characters.forEach(character => {
          ...
          favoriteButton.addEventListener("click", event => {
            ...
            addToFavorites(characterName, characterImageURL);
...
// Calling the function to fetch and display the character images on initial page load
...
function addToFavorites(superhero, characterImageURL) {
  let favorites = JSON.parse(localStorage.getItem("favorites")) || [];
  if (!favorites.some(item => item.superhero === superhero)) {
    const favoriteData = { superhero, characterImageURL };
...
const targetSuperhero = document.getElementById("main");
